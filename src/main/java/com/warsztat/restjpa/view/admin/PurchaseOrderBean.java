...
public class PurchaseOrderBean implements Serializable
{
   ...
   private Predicate[] getSearchPredicates(Root<PurchaseOrder> root)
   {
      ...
      {
         predicatesList.add(builder.equal(root.<User> get("user"), user));
      }     
      String street = this.example.getDeliveryAddress().getStreet();
      if (street != null && !"".equals(street))
      {
         predicatesList.add(builder.like(builder.lower(root.<String> get("deliveryAddress.street")), '%' + street.toLowerCase() + '%'));
      }
      String city = this.example.getDeliveryAddress().getCity();
      if (city != null && !"".equals(city))
      {
         predicatesList.add(builder.like(builder.lower(root.<String> get("deliveryAddress.city")), '%' + city.toLowerCase() + '%'));
      }
      String zipcode = this.example.getDeliveryAddress().getZipcode();
      if (zipcode != null && !"".equals(zipcode))
      {
         predicatesList.add(builder.like(builder.lower(root.<String> get("deliveryAddress.zipcode")), '%' + zipcode.toLowerCase() + '%'));
      }
